...
public class JoinOptimizer {
    ...
    public static OpIterator instantiateJoin(LogicalJoinNode lj,
        ...
        //For EC4: Checks the LogicalJoinNode to see what type of hash to use.
        if (lj.jt == LogicalJoinNode.JoinType.HASH)
    ...
    // added in this function for extra credit 4. Note that it takes advantage 
    // of caching in the bufferpool
    public double estimateHashJoinCost(LogicalJoinNode j, int card1, int card2,
            double cost1, double cost2) {
    	int tdsize = Database.getCatalog().getTupleDesc(
                this.p.getTableId(j.t1Alias)).getSize();
    	
    	int loads = tdsize*card1 / (BufferPool.DEFAULT_PAGES*BufferPool.getPageSize());
        return cost1 + card1 * cost2 + loads * card2;
    }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS) {
        	...
        	// this case is for extra credit 1.2
        	if ((t1pkey && t2pkey) || (!t1pkey && !t2pkey)) {
        		
                int t1_id = tableAliasToId.get(table1Alias);
                int t2_id = tableAliasToId.get(table2Alias);
                String t1_name = Database.getCatalog().getTableName(t1_id);
                String t2_name = Database.getCatalog().getTableName(t2_id);
            
                int field1 = Database.getCatalog().getTupleDesc(t1_id).fieldNameToIndex(field1PureName);
                int field2 = Database.getCatalog().getTupleDesc(t2_id).fieldNameToIndex(field2PureName);
                
        		int numDistinct1 = stats.get(t1_name).getDistinctValues(field1);
    			int numDistinct2 = stats.get(t2_name).getDistinctValues(field2);
    			
    			int maxNumDistinct = numDistinct1 > numDistinct2 ? numDistinct1 : numDistinct2;
    			
    			double selectivity = (1.0 / (double) (maxNumDistinct));
    	        card = (int) (card1 * card2 * selectivity);
        	} else if (t1pkey) {
        		card = card2;
        	} else if (t2pkey) {
        		card = card1;
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	for (int i = 1; i <= joins.size(); i++) {
    		for (Set<LogicalJoinNode> subset : enumerateSubsets(joins, i)) {
    			...
    			// set up costcard to hold solution
    			CostCard optimal = new CostCard();
    			optimal.plan = null;
    			optimal.cost = Double.MAX_VALUE;
    			optimal.card = Integer.MAX_VALUE;
    			
    			for (LogicalJoinNode n : subset) {
    				CostCard toTry = computeCostAndCardOfSubplan(
    						stats, filterSelectivities, n, subset, optimal.cost, cache);
    				if (toTry != null) {
    					if (toTry.cost < optimal.cost) {
    						optimal = toTry;
    					}
    				}
    				cache.addPlan(subset, optimal.cost, optimal.card, optimal.plan);
    			}
    		}
    		
    	}
    	Set<LogicalJoinNode> joinSet = new HashSet<LogicalJoinNode>(joins);
    	Vector<LogicalJoinNode> optimalJoinNode = cache.getOrder(joinSet);
    	
    	
    	if (explain) {
    		printJoins(optimalJoinNode, cache, stats, filterSelectivities);
    	}
    	
        return optimalJoinNode;
    }
    ...
    private CostCard computeCostAndCardOfSubplan(
        ...
        boolean isEquiJoin = j.p == Predicate.Op.EQUALS;
        
        // set as default for EC4.
        j.jt = LogicalJoinNode.JoinType.NESTED_LOOP;
        
        // For EC4: If it is an equality join, it also considers using a hash join instead of a nested join.
        // It picks the join with the lowest cost.
        if  (isEquiJoin) {
        	double cost1h = estimateHashJoinCost(j, t1card, t2card, t1cost, t2cost);
        	double cost2h = estimateHashJoinCost(j2, t1card, t2card, t1cost, t2cost);
        	double minHashCost = cost1h < cost2h ? cost1h : cost2h;
        	double minNLCost = cost1 < cost2 ? cost1 : cost2;
        	
        	if (minHashCost < minNLCost) {
        		j.jt = LogicalJoinNode.JoinType.HASH;
        		cost1 = cost1h;
        		cost2 = cost2h;
        	}
        }
        // we wrote the above code so that this part could remain as is
        if (cost2 < cost1) {
        	boolean tmp;
